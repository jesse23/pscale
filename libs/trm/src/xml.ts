...
import { Data, DataGraph, XMLOutput } from "./types";
import { fromJSON, setupGraph, toJSON } from "./graph";
...
export const fromXML = (
  ...
  parser.write(xml.join("")).close();

  // return ctx.path.pop() as DataGraph;
  return options.elem_as === "attr"
    ? fromJSON(proc.completeNode(), options.template || ["{ 'xml': [ds] }"])
    : setupGraph(graph);
};
 ...
 */
export const toXML = (ds: DataGraph, template: string[]): string[] => {
  const root = toJSON(ds, template) as XMLOutput;
