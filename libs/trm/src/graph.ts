import { Data, DataGraph, TrvDef } from "./types";
import { KEY_REFBY, KEY_TYPE } from "./const";
import { evalExpression, isArray } from "./utils";
...
/**
 * Convert JSON to data graph
 *
 * @param input JSON input
 * @param template template as string array
 * @returns data graph
 */
export const fromJSON = (ds: unknown, template: string[]): DataGraph =>
  setupGraph(
    (evalExpression(template.join("\n"), { ds }, true) || {}) as DataGraph
  );

/**
 * Convert data graph to JSON
 * @param ds data graph
 * @param template template as string array
 * @returns JSON object
 */
export const toJSON = (ds: DataGraph, template: string[]): unknown => {
  return evalExpression(template.join("\n"), { ds }, true);
};
